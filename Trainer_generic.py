...
from PixelHitsCNN.code.Trainer.models import architectures, losses
import plotting
...
class Trainer:
    ...
    def _ensure_model_loaded(self):
        ...
        #    self.model = tf.keras.models.load_model(self.model_dest, custom_objects=custom_objects,  safe_mode=False)
        #    return
    ...
    def prepare_train_test_input(self, train_split=0.9, seed=42):
        ...
        generic_x      = apply("Generic_x")
        generic_y      = apply("Generic_y")
    ...
    def train(self, validation_split=0.02):
        ...
        target_offset_train = 10 * target_offset_train 
        target_offset_test = 10 * target_offset_test
        input_len = x_train.shape[1]
        print(np.mean(target_offset_train[:,0]), np.std(target_offset_train[:,0]))
        exit()
        ...
        for ind in range(10):
            print(pred_train[ind, 0], pred_train[ind, 1])
        ...
        history_plot = f"plots/NNwithGeneric_{self.layer}_{self.axis}_history.png"
        plotting.plot_dnn_loss(history.history, history_plot)
        plotting.plot_nll_and_mse(history.history, f"plots/NNwithGeneric_{self.layer}_{self.axis}")
    ...
    def test(self):
        ...
        os.makedirs("plots", exist_ok=True)
        residuals_output_file = f"plots/NNwithGeneric_{self.layer}_{self.axis}_residuals.pdf"
        pulls_output_file = f"plots/NNwithGeneric_{self.layer}_{self.axis}_pulls.pdf"
        ...
        plotting.plot_uncertainties(uncertainties, f"plots/NNwithGeneric_{self.layer}_{self.axis}_uncertainties.pdf")

    def plot_otherMethods(self):
        simhit_y = self._first_simhit_per_event(self.simhit_y_test, "SimHit_y_train")[:,0]
        residuals_center_y = ( self.center_y_test - simhit_y ) * 1e4 
        residuals_generic_y = ( self.generic_y_test - simhit_y ) * 1e4
        ...
        residuals_center_x = ( self.center_x_test - simhit_x ) * 1e4 
        residuals_generic_x = ( self.generic_x_test - simhit_x ) * 1e4
