...
int main() {
    ...
    int batch_size = 300;
    int input_size = 21;
    std::string graph_path = "../PixelHitNN/checkpoints/L1F_y.keras.pb";
    ...
    tensorflow::Tensor input_tensor(
        tensorflow::DT_FLOAT,
        tensorflow::TensorShape({batch_size, input_size, 1})
    );
    ...
    tensorflow::Tensor angles_tensor(
        tensorflow::DT_FLOAT,
        tensorflow::TensorShape({batch_size, 2})
    );

    auto angles = angles_tensor.tensor<float, 2>();
	for (int n = 0; n < batch_size; n++){
	    angles(n, 0) = 0.0;
	    angles(n, 1) = 0.0;
	}
    tensorflow::Tensor charge_tensor(
        tensorflow::DT_FLOAT,
        tensorflow::TensorShape({batch_size, 1})
    );
    auto charge = charge_tensor.tensor<float, 2>();
	for (int n = 0; n < batch_size; n++){
    		charge(n, 0) = 1.0;
	}
    ...
    std::vector<std::pair<std::string, tensorflow::Tensor>> inputs = {
        {"pixel_projection_y:0", input_tensor},
        {"angles:0", angles_tensor},
        {"cluster_charge:0", charge_tensor}
    };
    ...
    std::ofstream file("output_y.txt");
